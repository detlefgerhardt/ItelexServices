	...
	public partial class MainForm : Form
	{
		...
		public MainForm()
		{
			...
			IncomingConnectionManagerConfig config = new IncomingConnectionManagerConfig()
			{
				...
				MonitorServerType = MonitorServerTypes.ItelexChatServer,
				MonitorPort = Constants.DEFAULT_MONITOR_PORT,
				ItelexExtensions = new ItelexExtensionConfiguration[]
				{
					new ItelexExtensionConfiguration(11, Constants.DEFAULT_NUMBER, "de", Constants.PROGRAM_SHORTNAME_DE, Constants.ANSWERBACK_DE, true),
					new ItelexExtensionConfiguration(12, Constants.DEFAULT_NUMBER, "en", Constants.PROGRAM_SHORTNAME_EN, Constants.ANSWERBACK_EN, false)
				}
			};
			MainServiceCtrl.StartIncomingConnections<IncomingChatConnectionManager>(config);
			MainServiceCtrl.StartOutgoingConnections<NotificationConnectionManager>();


			//BiorhythmusAction ba = new BiorhythmusAction(ActionBase.ActionCallTypes.Direct);
			//ba.Play();
		}

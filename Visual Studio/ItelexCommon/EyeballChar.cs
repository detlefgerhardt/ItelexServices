...
namespace ItelexCommon
{
	public class EyeballChar
	{
		...
		public byte[] GetPunchCodesFromAscii(string asciiText)
		{
			ShiftStates shiftState = ShiftStates.Unknown;
			byte[] baudotCode = CodeManager.AsciiStringToBaudot(asciiText, ref shiftState, CodeSets.ITA2);

			List<byte> eyeballCodes = new List<byte>();
			foreach (byte code in baudotCode)
			{
				...
				byte[] eyeball = GetPunchCodes(code, shiftState);
				if (eyeball != null && eyeball.Length > 0)
				{
					foreach (byte b in eyeball)
					{
						eyeballCodes.Add(b);
					}
				}
			}
			return eyeballCodes.ToArray();
		}
